...
var contentsEvents = __webpack_require__(9);
...
var onCreateContent = function onCreateContent(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.createContent(data).then(ui.createContentSuccess).catch(ui.createContentFail);
};

// THIS IS HANDLEBARS FOR LISTING RESOURCE

var onShowAllContents = function onShowAllContents(event) {
  event.preventDefault();
  api.showContents().then(ui.showContentsSuccess).catch(ui.showContentsFail);
};

// THIS IS HANDLEBARS FOR OPEN READ LISTING RESOURCE

var onShowAllContentsOp = function onShowAllContentsOp() {
  api.showContentsOp().then(ui.showContentsOpSuccess).catch(ui.showContentsOpFail);
};

// THIS IS FOR DELETING A CONTENT ON RESOURCE

var onDeleteShownContents = function onDeleteShownContents(event) {
  event.preventDefault();
  var contentsId = $(event.target).closest('button').attr('data-id');
  api.deleteContents(contentsId).then(function () {
    return onShowAllContents(event);
  }).catch(ui.showContentsFail);
};

// THIS IS FOR UPDATING A CONTENT ON RESOURCE

var onUpdateShownContents = function onUpdateShownContents(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var contentsId = $(event.target).attr('data-id');
  api.updateContents(contentsId, data).then(function () {
    return onShowAllContents(event);
...
module.exports = {
  ...
  onShowAllContents: onShowAllContents,
  onShowAllContentsOp: onShowAllContentsOp,
  onDeleteShownContents: onDeleteShownContents,
  onUpdateShownContents: onUpdateShownContents
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
...
var showContents = function showContents(data) {
  return $.ajax({
    method: 'GET',
    url: config.apiUrl + '/contents',
    data: data,
...
var showContentsOp = function showContentsOp(data) {
  return $.ajax({
    method: 'GET',
    url: config.apiUrl + '/contents',
    data: data
...
var deleteContents = function deleteContents(contentsId) {
  return $.ajax({
...
var updateContents = function updateContents(contentsId, data) {
  return $.ajax({
    url: config.apiUrl + '/contents/' + contentsId,
    method: 'PATCH',
    data: data,
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://quiet-ravine-35247.herokuapp.com',
...
var store = __webpack_require__(3);
var showContentsTemplate = __webpack_require__(13);
var showOpContentsTemplate = __webpack_require__(30);
...
var createContentFail = function createContentFail(data) {
  $('#create-content-message').html('Your question was not successfully created');
};

// SHOW LISTING IF CONTENT

var showContentsSuccess = function showContentsSuccess(data) {
  if (data.contents.length !== 0) {
    var showContentsHtml = showContentsTemplate({ contents: data.contents });
...
var showContentsOpSuccess = function showContentsOpSuccess(data) {
  if (data.contents.length !== 0) {
    var showOpContentsHtml = showOpContentsTemplate({ contents: data.contents });
...
var showContentsFail = function showContentsFail(error) {
  console.error(error);
};

// SHOW ONE OPENE READ CONTENT FORM RESOURCE ERROR

var showContentsOpFail = function showContentsOpFail(error) {
  console.error(error);
};
...
module.exports = {
  ...
  createContentFail: createContentFail,
  showContentsSuccess: showContentsSuccess,
  showContentsFail: showContentsFail,
  showContentsOpSuccess: showContentsOpSuccess,
  showContentsOpFail: showContentsOpFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <div class=\"handlebar\">\n\n    <form class=\"form-update-content\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n\n      <div>\n\n        <label>Update Question</label>\n        <input type=\"text\" name=\"content[question]\" placeholder=\"Ask anything!\" required=\"true\">\n\n      </div>\n\n      <div>\n\n        <button data-id=\"update-content\" type=\"submit\" class=\"btn btn-primary\">Update your Question</button>\n\n        <button class=\"btn btn-default remove-button\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
...
module.exports = __webpack_require__.p + "2ab764c56d1fd0683aa4ed7b3df556b4.jpg";
